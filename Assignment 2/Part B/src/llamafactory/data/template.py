...
register_template(
    ...
    format_user=StringFormatter(slots=[
        "{{instruction}}\n\n"  # 固定指令
        "【相关法律条文】\n{{input.split('<问题>')[0].strip()}}\n\n"  # 自动提取法条
        "【待解答问题】\n{{input.split('<问题>')[1].strip()}}"  # 自动提取问题
    ]),
    format_assistant=StringFormatter(slots=[
        "【法律分析】\n{{output}}"  # 结构化输出
    ]),
    default_system=(
        "你是一名专业中国法律顾问，回答问题需满足以下要求：\n"
        "1. 必须引用具体的法律条文（格式：《XXX法》第XX条）\n"
        "2. 法律分析需结合条文与问题情境，与问题必须直接相关\n"
        "3. 结论应当明确且具有可执行性，结论应当清晰明确，不得含混其词\n"
    ),
    stop_words=["<问题>", "\n法律依据：", "<|endoftext|>"],
    efficient_eos=True
)
